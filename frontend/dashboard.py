...
prev_messages = [
    {"content": f"Message {i}", "from_user": i % 2 == 0} for i in range(100)
]
radar_data = []
LOAD_SIZE = 10
n_top_candidates = 4
...
async def serve(q: Q):
    if not q.client.initialized:
        ...
        q.page["header"] = ui.header_card(
            ...
            subtitle="DSA4213 Group Whisper",
                ...
                data=da(
                    ...
                    rows=[
                        (
                            ...
                            radar_data[i - 1]["apiDesignExperience"],
                        ),
                        (
                            "Framework knowledge",
                            radar_data[i - 1]["frameworkKnowledge"],
                        ),
                        ("Databases skill", radar_data[i - 1]["databaseSkill"]),
                        (
                            "Cybersecurity knowledge",
                            radar_data[i - 1]["cybersecurityKnowledge"],
                        ),
        ...
        q.page["table"] = ui.form_card(
            ...
            items=[
                ui.table(
                    name="table",
                    columns=[
                        ui.table_column(name=n, label=l, filterable=True)
                        for l, n in labels.items()
                    ],
                    rows=[
                        ui.table_row(
                            name=f"row{i}",
                            cells=[str(row[label]) for label in labels.values()],
                        )
        ...
        q.page["exp_dits"] = ui.wide_pie_stat_card(
            box="lbottom",
            ...
            for link in links:
                ...
                with open(local_path, "rb") as file:
                    ...
                    response = requests.post(
                        "http://localhost:4000/file/upload", files=files
                    )
                print(response.text)

                items.append(
                    ui.link(
                        label=f"{os.path.basename(link)} ({size} bytes)",
                        download=True,
                        path=link,
                    )
                )
        ...
        if q.args.chatbot:
            ...
            # Stream bot response.
            q.client.task = asyncio.create_task(
                stream_chatbot_response(q.args.chatbot, q)
            )
        ...
        if q.events.chatbot and q.events.chatbot.scroll_up:
            if q.client.current_load_page == 0:
                ...
                q.page["chat_bot"].prev_items = prev_messages[
                    end : q.client.current_load_page
                ]
